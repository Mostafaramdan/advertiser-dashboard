...
import type { CouponItem } from '../interfaces/Coupons'
/***************************************
...
<template>
  <VExpansionPanels :model-value="0" class="mt-5">
    <VExpansionPanel>
      <VExpansionPanelTitle>الكوبونات الأكثر إستخداما</VExpansionPanelTitle>
      <VExpansionPanelText>
        <v-table class="simple-table" density="compact" v-if="stats.length">
          ...
          <tbody>
            <tr v-for="item in stats" :key="item.id">
              ...
              <td>{{ item.type }}</td>
              <td>
                <span class="max-width-180">{{ item.store_name || '-' }}</span>
              </td>
              <td>
