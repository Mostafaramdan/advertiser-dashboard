...
const isValidForm = computed(() => {
  return productList.value.some((product: OfferProduct) => product.accepted !== null)
})
...
// #region Functions
function submit() {
  const payload = productList.value
    .filter((product: OfferProduct) => product.accepted !== null)
    .map((product: OfferProduct) => ({
      id: product.id,
  ...
  offersService
    .offerProductsAcceptance(props.offerId, payload)
    .then((res) => {
  ...
  offersService
    ...
    .then((res) => {
      productList.value = res.data.data.map((item: any) => ({
        ...item,
        accepted: item.status === 'accepted' ? true : item.status === 'rejected' ? false : null,
      }))
    })
    .finally(() => {
...
<template>
  <VDialog v-model="showModal" max-width="600" persistent>
    ...
    <!-- Dialog Content -->
    <VCard>
      <VCardTitle class="py-3">الموافقة علي منتجات العرض</VCardTitle>
        ...
        <VList :lines="false" v-if="productList.length" class="py-0">
          <VListItem border v-for="product in productList" :key="product.id" class="text-sm">
            <VRow class="align-center my-0">
              ...
              <VCol cols="12" sm="4" class="d-flex justify-sm-end py-1">
                <VBtnToggle
                  v-model="product.accepted"
                  color="primary"
                  variant="outlined"
      ...
      <VCardText class="d-flex justify-end flex-wrap gap-3">
        ...
        <VBtn
          ...
          :disabled="isLoading.submit || !isValidForm"
          @click="submit"
        >
          {{ t('actions.save') }}
        </VBtn>
