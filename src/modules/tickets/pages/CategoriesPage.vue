...
const MODEL_NAME = 'ticket_categories'
...
const permissions = computed(() => ({
  create: hasPermission('create_ticket_category'),
  edit: hasPermission('update_ticket_category'),
  delete: hasPermission('delete_ticket_category'),
  changeStatus: hasPermission('change_status_ticket_category'),
  sort: hasPermission('sort_ticket_category'),
}))
...
<template>
  ...
  <VCard title="أقسام التذاكر / موظفي الدعم" class="page-card">
    <VCardText>
      <PageActions
      ...
      >
        ...
        <template #item.user_types="{ item }">
          <div class="d-flex" v-if="item.raw.user_types">
            <VChip variant="outlined" color="primary" label>
              {{ USERS_TYPES[item.raw.user_types[0]] }}
            </VChip>
