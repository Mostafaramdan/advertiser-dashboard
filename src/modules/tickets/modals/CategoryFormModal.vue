...
const formData = reactive<Category | CategoryFormData>({
  name: '',
  user_types: [],
  admins: [],
  is_active: true,
...
function getAdminsList() {
  ...
  listService
    .getAdmins()
    .then((res: any) => {
      ticketsStore.setAdminsList(res.data.data)
    })
    .finally(() => {
...
function create() {
  categoriesService
    .createItem(formData as CategoryFormData)
    .then((res) => {
      toast.success(res.data.message)
            ...
            <VRow>
              ...
              <VCol cols="12">
                <AppAutocomplete
                  ...
                  name="user_types"
                  :items="getOptionsArrayFromObject(USERS_TYPES)"
                  item-title="label"
                  item-value="value"
                  label="نوع المستخدم"
                  rules="required"
